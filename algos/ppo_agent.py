...
class PPO(OnPolicyAlgorithm, InferenceInterface):
    """
    ...
    def __init__(
        ...
        lr_value: Union[float, Schedule] = 1e-3,
        ...
        :param lr_value: The learning rate, it can be a constant or a function
            of the current progress remaining (from 1 to 0). Defaults to 1e-3.
        ...
        :param clip_range: Clipping parameter, it can be a function of the current
            progress remaining (from 1 to 0). Defaults to 0.2.
        :param clip_range_vf: Clipping parameter for the value function, it can be a
        ...
        :param target_kl: Limit the KL divergence between updates. By default,
            there is no limit on the KL divergence, if specified can it can be used to stop the updates (commented out as less effective) on current rollout or adapt the learning rate. Defaults to None.
        ...
        :param rollout_buffer_class: Rollout buffer class to use. If None, uses
            default RolloutBuffer. Defaults to None.
    ...
    def predict_rollout(
        ...
        """
        if "buffer_size" in self.rollout_buffer_kwargs: del self.rollout_buffer_kwargs["buffer_size"]
        rollout_buffer = eval(self.rollout_buffer_class)(  # type: ignore[assignment]
            buffer_size=self.n_steps-1,
            observation_space=self.observation_space,
            action_space=self.action_space,
            device=self.device,
            gamma=self.gamma,
    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            if terminal_mask.any():
                ...
                with torch.no_grad():
                    terminal_values = self.policy.predict_values(terminal_obs)  # type: ignore[arg-type]
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(batch_size//self.num_mini_batch):
                with self.accelerator.accumulate(self.policy):
                    ...
                    _, values, dist_out, entropy = self.policy.evaluate_actions(rollout_data.observations, actions)
                    ...
                    if self.target_kl is not None and self.lr_scheduler == "adaptive":
                        if kl > self.target_kl * 2.0:
                            self.lr_value = max(1e-5, self.lr_value / 1.5)
                        elif (kl < self.target_kl / 2.0) and (kl > 0.0):
                            self.lr_value = min(1e-2, self.lr_value * 1.5)
                        self._update_learning_rate(self.policy.optimizer)
                    ...
                    if self.accelerator.sync_gradients:
                        ...
                        # self.logger.record_mean("train/efficiency_loss", efficiency_loss.item())
    ...
    def _excluded_save_params(self) -> List[str]:
        return ["observation_space", "action_space"] + super()._excluded_save_params()
