...
from algos.policies.ac_lstm_policy import MlpLstmPolicy, RecurrentActorCriticPolicy
from algos.storage.buffers import RecurrentRolloutBuffer, RolloutBuffer
...
class RecurrentPPO(OnPolicyAlgorithm, InferenceInterface):
    """
    Proximal Policy Optimization algorithm (PPO) (clip version)
    with support for recurrent policies (e.g. LSTM).
    ...
    def __init__(
        ...
        :param policy: The recurrent policy model to use (str or Type[RecurrentActorCriticPolicy]).
        ...
        :param mixed_precision: Precision mode for training (e.g., 'fp16', 'bf16').
            Defaults to None.
        :param rollout_buffer_class: Rollout buffer class to use. If None, uses
            default RecurrentRolloutBuffer. Defaults to None.
    ...
    def predict_rollout(
        ...
        with torch.no_grad():
            while not (episodes_completed >= n_episodes).all():
                actions, memory = self.policy.predict(last_obs, self._last_memory)
                ...
                self._last_memory[dones] = 0  # Note: the policy memory may necessitate custom init
    ...
    def collect_rollouts(
        ...
        rollout_buffer.restart_memory(torch.ones(self.n_envs).long(), self._last_memory)
        callback.on_rollout_start()
        ...
        while n_steps < n_rollout_steps:
            ...
            with torch.no_grad():
                actions, values, distribution_output, memory = self.policy(self._last_obs, self._last_memory)
            ...
            if terminal_mask.any():
                ...
                with torch.no_grad():
                    terminal_values = self.policy.predict_values(
                        terminal_obs,
                        memory[terminal_mask] if memory is not None else None,
            ...
            rollout_buffer.restart_memory(dones)  # Note: the policy memory may necessitate custom init

            # Go on with next step
            self._last_obs = obs
            self._last_memory = rollout_buffer.get_last()
        ...
        with torch.no_grad():
            values = self.policy.predict_values(obs, self._last_memory)
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(batch_size//self.num_mini_batch):
                with self.accelerator.accumulate(self.policy):
                    ...
                    # Normalize advantage; ignore if `mini_batch_size` == 1
                    ...
                    if self.accelerator.sync_gradients:  # note: consider that if 'sync on datalaoder end' is disabled we risk to leave grad not updated
                        ...
                        self.logger.record_mean("train/clip_fraction", clip_fraction.item())
                        self.logger.record_mean("train/kl", kl.item())
    ...
    def _excluded_save_params(self) -> List[str]:
        return ["observation_space", "action_space", "_last_memory"]+super()._excluded_save_params()  # noqa: RUF005
